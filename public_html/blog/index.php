...
foreach ($visible as $index => $post) {
    $postUrl = RELET_SITE_URL . '/blog/' . $post['slug'] . '/';
    $items[] = [
        '@type' => 'ListItem',
        'position' => $index + 1,
        'url' => $postUrl,
...
?><main id="main"><section class="page-hero"><div class="container"><div class="breadcrumbs"><a href="/">Home</a> / Advice</div><p class="eyebrow">Property notes</p><h1>Property maintenance advice for Nottingham landlords and letting agents.</h1><p class="lead">Practical guidance on landlord repairs, void property turnarounds, cleaning, joinery and rental-property decisions.</p></div></section><section class="section"><div class="container"><?php if ($visible === []): ?><div class="card"><h2>No published articles yet</h2><p>New practical property guidance will appear here when it is published.</p></div><?php else: ?><div class="grid-3"><?php foreach ($visible as $post): ?><article class="card"><span class="tag">Property advice</span><h2 style="font-size:1.6rem"><a href="/blog/<?= relet_escape($post['slug']) ?>/"><?= relet_escape($post['title']) ?></a></h2><p><?= relet_escape($post['excerpt']) ?></p><a class="link" href="/blog/<?= relet_escape($post['slug']) ?>/">Read article →</a></article><?php endforeach; ?></div><?php endif; ?><?php if ($totalPages > 1): ?><nav class="pagination" aria-label="Blog pages"><?php if ($page > 1): ?><a class="btn btn-outline" href="?page=<?= $page - 1 ?>">Previous</a><?php endif; ?><span>Page <?= $page ?> of <?= $totalPages ?></span><?php if ($page < $totalPages): ?><a class="btn btn-outline" href="?page=<?= $page + 1 ?>">Next</a><?php endif; ?></nav><?php endif; ?></div></section><section class="section section-soft"><div class="container"><div class="section-head"><div><p class="eyebrow">Case studies</p><h2>Property work in practice</h2></div><a class="link" href="/case-studies.html">Nottingham property maintenance case studies →</a></div><p>Looking for practical help now? Explore <a href="/landlord-repairs.html">landlord property repairs in Nottingham</a>, <a href="/void-turnarounds.html">void property turnarounds</a> or <a href="/letting-agents.html">property maintenance for Nottingham letting agents</a>.</p></div></section></main><?php relet_public_footer();
